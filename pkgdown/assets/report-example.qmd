...
- 🔍 [Effect Sizes in t-tests and F-tests](#effect-sizes-in-t-tests-and-f-tests){.yellow}: We found 1 t-test and/or F-test where effect sizes are not reported. Check these tests in the table below, and consider adding effect sizes  
...
"AsPredicted")), row.names = c(NA, -2L), class = "data.frame")
...
"2025-08-05T11:55:35.267689", "2025-11-28T19:14:24.608826", "", 
...
"to_err_is_human")), row.names = c("template_name", "title", 
...
"research_questions", "paper_id"), class = "data.frame")
...
"unpaired t-test"), sample_size = c(30L, 50L, 50L), alpha_level = c(0.05, 
NA, NA), power = c(0.8, 0.8, 0.8), effect_size = c(NA, 0.5, NA
), effect_size_metric = c("Cohen's d", "Cohen's d", NA), software = c("G*Power", 
...
<p class='validation'>**Validation**: In a sample of 128 papers with 246 instances of power statements, 203 were correctly detected (true positives), 22 were missed (false negatives) and 21 were incorrectly detected (false positives). Overall, among all instances flagged as power statements, 90.6% were correct (positive prediction value).</p>
...
<p class='validation'>**Validation**: In a sample of 194 papers with 1602 instances of non-significant p-values, this module correctly detected 1486 of them, and incorrectly identified 153. Additionally, 91% of detections were true instances (positive predictive value). That is, when this module flags non-significant p-values in a paper, it correctly identifies an issue 91% of the time.</p>
...
<p class='validation'>**Validation**: In a sample of 51 papers with 87 statements, this module correctly identified 38 statements (true positives) and incorrectly flagged 22 statements (false positives). It failed to detect 27 statements. Thus, among all statements flagged by the module, 63% were genuine cases (positive predictive value). However, the module missed 42% of all true statements (false negative rate).</p>

### 🔍 Effect Sizes in t-tests and F-tests {#effect-sizes-in-t-tests-and-f-tests .yellow}

We found 1 t-test and/or F-test where effect sizes are not reported. Check these tests in the table below, and consider adding effect sizes

<details><summary>View detailed feedback</summary><div>

We recommend checking the sentences below, and add any missing effect sizes.

For t-tests with a reported d, coherence checks yielded 1 indeterminate case.
...
"t(97.2) = -1.96"), "Test Type" = c("t-test", "t-test"), "d Coherence" = c("indeterminate", 
"indeterminate"), "d Assumption" = c("none", "none"), "d Coherence Note" = c("Non-integer df indicates Welch's t-test (unequal variances); sample sizes cannot be determined.", 
"No parseable d effect size found."), "eta Coherence" = c(NA_character_, 
NA_character_), "eta Assumption" = c(NA_character_, NA_character_
), "eta Coherence Note" = c(NA_character_, NA_character_)), row.names = c(NA, 
-2L), class = c("tbl_df", "tbl", "data.frame"))
...
# table data --------------------------------------
table <- structure(list("File name" = c("test_script.R", "bad.R", "bad.Rmd", 
"Code/Study 1.r"), "Absolute paths found" = c("/lisa/file.csv<br>C:/lisa/file.csv", 
"C://Lakens/files/example.csv", "C://Lakens/files/example.csv<br>C://Lakens/files/example2.csv", 
"/lisa/file.csv<br>C:/lisa/file.csv")), row.names = c(NA, 4L), class = "data.frame")
